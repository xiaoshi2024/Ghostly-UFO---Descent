...
public class UfoL_blockModel  extends DefaultedBlockGeoModel<UfoL_blockentity> {
    public UfoL_blockModel() {
        super(ResourceLocation.fromNamespaceAndPath(GhostlyUFODescent.MODID, "ufo_l"));
    }

    // We add the `examplemod_day_time` DataTicket so it can be used later
    @Override
    public void addAdditionalStateData(UfoL_blockentity animatable, GeoRenderState renderState) {
        renderState.addGeckolibData(UfoL_blockentity.DAY_TIME, animatable.getLevel().dayTime());
    }
